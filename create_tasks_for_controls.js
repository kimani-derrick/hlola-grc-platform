...
async function getTasksForControl(controlId) {
  try {
    const response = await axios.get(`${API_BASE}/tasks/controls/${controlId}`, { headers });
    return response.data.tasks || [];
  } catch (error) {
    console.error(`Error fetching tasks for control ${controlId}:`, error.message);
    return [];
  }
}
...
async function createTaskForControl(control) {
  ...
  try {
    ...
    console.error(`Error creating task for control ${control.id}:`, error.message);
    return null;
  }
}
...
async function main() {
  ...
  console.log('📋 Controls without tasks:');
  controlsWithoutTasks.forEach((control, index) => {
    console.log(`${index + 1}. ${control.control_id} - ${control.title} (${control.framework_name})`);
  });

  console.log('\n🚀 Creating tasks for controls without tasks...\n');

  let successCount = 0;
  let errorCount = 0;

  // Create tasks for controls without tasks (limit to first 10 for demo)
  const controlsToProcess = controlsWithoutTasks.slice(0, 10);
  
  for (const control of controlsToProcess) {
    console.log(`Creating task for: ${control.control_id} - ${control.title}`);
  ...
  console.log(`📋 Total controls processed: ${controlsToProcess.length}`);
  console.log(`🔄 Remaining controls without tasks: ${controlsWithoutTasks.length - controlsToProcess.length}`);
}
