...
class ComplianceEventListener {
  ...
  static async onFrameworkAssigned(entityId, frameworkId) {
    try {
      ...
      // Note: Task assignments are now handled by taskAssignmentHelper.createTaskAssignmentsForEntity()
      // in the controller, not here. This separation keeps compliance logic separate from task management.
      
      // Step 2: Run compliance check for the new framework
  ...
  static async copyFrameworkTasks(entityId, frameworkId) {
    try {
      ...
      // Create tasks for each control using the new architecture
      let tasksCreated = 0;
      for (const control of controlsResult.rows) {
        try {
          // Check if task assignment already exists for this control and entity
          const existingAssignment = await pool.query(`
          ...
          if (existingAssignment.rows.length > 0) {
            logger.debug('Task assignment already exists for control', {
              entityId,
              controlId: control.id,
              controlTitle: control.title
            });
          ...
          // Create new task using the Task model (org-independent)
          const Task = require('../models/Task');
          ...
          // Create task assignment for the entity
          await Task.assignToEntity(task.id, entityId, null, control.priority || 'medium', taskDetails.dueDate);
          
          tasksCreated++;
          logger.debug('Task and assignment created for control', {
            entityId,
            controlId: control.id,
            controlTitle: control.title,
